...
__global__ void
downsweep_kernel(int N, int* result, int twod) {
...
__global__ void
upsweep_kernel(int N, int* result, int twod) {
    ...
    if (i+twod1 == N && twod1 >= N) {
        result[i+twod1-1] = 0;
    } 
    else if (i+twod1-1 < N && i % twod1 == 0) {
...
void exclusive_scan(int* device_start, int length, int* device_result)
{
    ...
    for (int twod = 1; twod < N; twod*=2) {
        upsweep_kernel<<<blocks, threadsPerBlock>>>(N, device_result, twod);
    }

    //downsweep
    int rounded_N = nextPow2(N);
    for(int twod = rounded_N/2; twod>= 1; twod /= 2){
        downsweep_kernel<<<blocks, threadsPerBlock>>>(N, device_result, twod);
    }
}
...
__global__ void
repeat_ids_kernel(int N, int* input, int* output) {
    // compute overall index from position of thread in current block,
    // and given the block we are in
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void
repeat_ones_kernel(int N, int* input, int* output) {
    // compute overall index from position of thread in current block,
    // and given the block we are in
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void
create_result_kernel(int N, int* repeat_ids, int* scan, int* result) {
    // compute overall index from position of thread in current block,
    // and given the block we are in
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    
    if (i < N && repeat_ids[i] >= 0) {
        result[scan[i]] = repeat_ids[i];
    }
}
...
int find_repeats(int *device_input, int length, int *device_output) {
    ...
    int* device_ids;
    cudaMalloc(&device_ids, length * sizeof(int));

    repeat_ids_kernel<<<blocks, threadsPerBlock>>>(length, device_input, device_ids);
    repeat_ones_kernel<<<blocks, threadsPerBlock>>>(length, device_ids, device_input);

    exclusive_scan(device_input, length, device_input);

    int* result_length = (int *)malloc(sizeof(int));
    create_result_kernel<<<blocks, threadsPerBlock>>>(length, device_ids,
                                         device_input, device_output);
    cudaMemcpy(result_length, device_input + (length-1), sizeof(int), cudaMemcpyDeviceToHost);
